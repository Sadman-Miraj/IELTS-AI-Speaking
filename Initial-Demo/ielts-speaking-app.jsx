...
function speak(text, cb) {
  ...
  const doSpeak = () => {
    ...
    // Failsafe in case onend doesn't fire (common on mobile)
...
export default function App() {
  ...
  // Refs (stable values for use inside callbacks/intervals)
  const stepsR    = useRef([]);
  ...
  const startMic = useCallback(() => {
    ...
    if (!SR) { addMsg({ role: "system", text: "⚠️ Speech recognition not available. Please use Chrome (Android or desktop)." }); return; }
    const r = new SR();
...
- Part 3: Abstract, societal, analytical (clearly harder than Part 1, e.g. "To what extent…" or "How do you think…")
- All questions must sound natural and be authentic IELTS-style`;
  ...
  const doFeedback = useCallback(async () => {
    ...
    const prompt = `Evaluate this IELTS Speaking test transcript. Answers were captured via speech-to-text, so minor transcription errors may exist — focus on language quality.
...
{
  "overallBand": 6.0,
  "examinerNote": "2 specific sentences about the candidate's overall performance, referencing actual answers.",
  "criteria": {
    "fluency": {
      "band": 6.0,
      "label": "Fluency & Coherence",
      "icon": "💬",
      "comment": "Specific 2–3 sentence assessment referencing what they actually said.",
    ...
    "lexical": {
      ...
      "icon": "📖",
      "comment": "Specific 2–3 sentence assessment.",
      "tips": ["Tip 1","Tip 2","Tip 3"],
      "upgrades": [
        {"used":"phrase from their actual answer","better":"more sophisticated or natural alternative"}
      ]
    },
    "grammar": {
      "band": 6.0,
      ...
      "tips": ["Tip 1","Tip 2","Tip 3"],
      "corrections": [
        {"wrong":"error they actually made","correct":"corrected version","rule":"brief grammar rule explanation"}
      ]
    },
    "pronunciation": {
      "band": 6.0,
      "label": "Pronunciation",
      "icon": "🎙️",
      "comment": "Specific 2–3 sentence assessment based on word patterns visible in transcript.",
      "tips": ["Tip 1","Tip 2","Tip 3"]
    }
  },
  "strengths": ["Specific strength 1 referencing actual response","Specific strength 2","Specific strength 3"],
  "improvements": ["Priority improvement 1","Priority improvement 2","Priority improvement 3"],
  "naturalSpeech": [
    {
      "context": "When asked about [topic]",
      "candidate": "what they actually said",
      "natural": "more fluent, natural, higher-band version",
      "why": "brief explanation of why this is better"
    }
  ],
  "closing": "A warm, specific, encouraging message that references something they did well."
}

Give realistic band scores. Most non-native speakers score 5.0–7.0. Do NOT give 8+ unless the transcript clearly shows near-native fluency. Be specific, not generic.`;

    try {
      setLoadMsg("Generating your feedback report…");
        ...
        <div style={{ maxWidth: 680, margin: "0 auto" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            <div style={{
              ...
              background: "#141B24", border: "1px solid #1E2D40",
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 28 }}>
            {[
              { label: "Part 1", sub: "Interview", color: "#60A5FA", time: "~5 min" },
              { label: "Part 2", sub: "Long Turn", color: "#C084FC", time: "~4 min" },
              { label: "Part 3", sub: "Discussion", color: "#34D399", time: "~5 min" },
            ].map((p) => (
              <div key={p.label} style={{
                flex: 1, background: "#141B24", border: "1px solid #1E2D40",
          ...
          {/* Section label */}
          <div style={{ color: "#475569", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 12 }}>
            ...
            {TOPICS.map((t) => {
              ...
              return (
                <button key={t.id} onClick={() => { setTopicId(t.id); setFreestyle(false); }}
                  style={{
                    background: active ? "#162033" : "#141B24",
                    border: `1px solid ${active ? "#60A5FA" : "#1E2D40"}`,
                    borderRadius: 12, padding: "14px 8px", cursor: "pointer",
                    color: "#E2E8F0", textAlign: "center", transition: "all .15s",
                    outline: "none",
                  }}>
          ...
          <button onClick={() => { setFreestyle(true); setTopicId(null); }}
            style={{
              ...
              background: freestyle ? "#1A1440" : "#141B24",
              border: `1px solid ${freestyle ? "#C084FC" : "#1E2D40"}`,
              color: "#E2E8F0", fontSize: 14, fontWeight: 600, marginBottom: 20,
              display: "flex", alignItems: "center", justifyContent: "center", gap: 10, outline: "none",
            }}>
          ...
          {/* Start CTA */}
          <button
            disabled={!topicId && !freestyle}
            onClick={() => generateExam(topicId, freestyle)}
            style={{
              width: "100%", padding: 18, borderRadius: 14, fontSize: 17, fontWeight: 800,
              cursor: !topicId && !freestyle ? "not-allowed" : "pointer",
              background: !topicId && !freestyle
                ? "#1E2D40"
                : "linear-gradient(135deg, #3B82F6 0%, #8B5CF6 100%)",
              color: !topicId && !freestyle ? "#475569" : "#fff",
              border: "none", letterSpacing: 0.3, transition: "opacity .2s", outline: "none",
            }}>
          ...
          <div style={{ marginTop: 16, padding: 12, background: "#141B24", borderRadius: 10, border: "1px solid #1E2D40", textAlign: "center" }}>
            <p style={{ margin: 0, color: "#475569", fontSize: 12 }}>
              🎙️ Requires <strong style={{ color: "#94A3B8" }}>Google Chrome</strong> on Android or desktop · Allow microphone when prompted
            </p>
          </div>
        </div>
      </div>
      ...
      <div style={{ ...app, display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ textAlign: "center", padding: 40 }}>
          <div style={{
            ...
            fontSize: 36, margin: "0 auto 20px", animation: "spin 2.5s linear infinite",
          }}>👩‍💼</div>
      ...
      <div style={{ ...app, display: "flex", flexDirection: "column", height: "100vh", maxHeight: "100dvh" }}>
        ...
        <div style={{ background: "#0E1520", borderBottom: "1px solid #1E2D40", padding: "10px 16px", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <div style={{ color: pColor, fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.6 }}>
                {PL[currPart]}
              </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              {showTimer && (
                <div style={{
                  background: timer < 15 ? "#450A0A" : "#141B24",
                  color: timer < 15 ? "#F87171" : "#E2E8F0",
                  border: `1px solid ${timer < 15 ? "#7F1D1D" : "#1E2D40"}`,
              ...
              <div style={{
                ...
                background: isListen && micOn ? "#4ADE80" : isTalking ? "#C084FC" : "#1E2D40",
                boxShadow: isListen && micOn ? "0 0 10px #4ADE8060" : "none",
                animation: (isListen && micOn) ? "pulse 1.5s ease-in-out infinite" : "none",
              }} />
            </div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "14px 14px 6px" }}>
          {msgs.map((m, mi) => {
            ...
            if (m.role === "cue-card") return (
              <div key={m.key} style={{
                background: "linear-gradient(135deg, #1A1F3A, #1E2A48)",
                border: "2px solid #4F46E5", borderRadius: 16, padding: 18,
                margin: "12px 0", animation: "slideUp .3s ease",
              }}>
                <div style={{ color: "#818CF8", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1.2, marginBottom: 10 }}>
                  📋 Part 2 — Task Card
                </div>
                <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 14, lineHeight: 1.4 }}>
                  {m.card.title}
                </div>
                <div style={{ color: "#94A3B8", fontSize: 12, marginBottom: 8, fontStyle: "italic" }}>You should say:</div>
                {m.card.points.map((p, i) => (
                  <div key={i} style={{ fontSize: 14, color: "#E2E8F0", padding: "3px 0", lineHeight: 1.6 }}>{p}</div>
                ))}
                {m.card.closing && (
                  <div style={{ color: "#818CF8", fontSize: 13, marginTop: 10, fontStyle: "italic" }}>…{m.card.closing}</div>
                )}
              </div>
            );
            ...
            if (m.role === "system") return (
              <div key={m.key} style={{ textAlign: "center", padding: "6px 0", color: "#FBBF24", fontSize: 12 }}>
                {m.text}
              </div>
            );
            ...
            return (
              <div key={m.key} style={{
                display: "flex", justifyContent: isExaminer ? "flex-start" : "flex-end",
                ...
                <div style={{
                  maxWidth: "76%", padding: "11px 15px", lineHeight: 1.65, fontSize: 14,
                  borderRadius: 16,
                  borderBottomLeftRadius: isExaminer ? 4 : 16,
                  borderBottomRightRadius: isUser ? 4 : 16,
                  background: isExaminer ? "#1A2744" : isLive ? "#0B1F14" : "#0D3620",
                  border: isLive ? "1px dashed #4ADE8055" : "none",
                  color: isLive && !liveText ? "#475569" : "#E2E8F0",
                }}>
                  {isExaminer && (
                    <div style={{ fontSize: 10, color: "#818CF8", fontWeight: 700, marginBottom: 5, textTransform: "uppercase", letterSpacing: 0.6 }}>
                      Examiner Sarah
                    </div>
                  )}
        ...
        <div style={{ background: "#0E1520", borderTop: "1px solid #1E2D40", padding: "14px 16px 18px", flexShrink: 0 }}>
          ...
          {isListen && (
            <div>
              {liveText && (
                <div style={{
                  background: "#0A1220", border: "1px solid #1E2D40", borderRadius: 10,
                  padding: "8px 12px", marginBottom: 10, fontSize: 13, color: "#94A3B8",
                  maxHeight: 64, overflowY: "auto", lineHeight: 1.6,
                }}>
                  {liveText}
                </div>
              )}
              <div style={{ display: "flex", gap: 10 }}>
                <div style={{
                  ...
                  background: "#0A1220", borderRadius: 12, padding: "12px 16px",
                  border: "1px solid #163220",
                }}>
                  <div style={{
                  ...
                  <span style={{ color: "#4ADE80", fontSize: 13, fontWeight: 600 }}>
                    {micOn ? "Recording — speak now" : "Starting microphone…"}
                  </span>
                </div>
                <button onClick={() => submitAnswer()}
                  style={{
                    background: "#3B82F6", color: "#fff", border: "none",
                    borderRadius: 12, padding: "0 22px", fontWeight: 700,
          ...
          {isPrep && (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              <div style={{ color: "#94A3B8", fontSize: 13, lineHeight: 1.5 }}>
                📝 Think about what you'll say. Take notes if you need.
              </div>
              <button
                onClick={() => {
                  clearInterval(timerR.current);
                  setPhase("idle"); phaseR.current = "idle";
                  const gm = "Alright, please begin speaking now.";
                  addMsg({ role: "examiner", text: gm, part: 2 });
                  speak(gm, () => procRef.current?.(stepIdxR.current + 1));
                }}
                style={{
                  background: "#141B24", color: "#94A3B8", border: "1px solid #1E2D40",
                  borderRadius: 8, padding: "7px 14px", fontSize: 12, cursor: "pointer",
                  flexShrink: 0, fontWeight: 600, outline: "none",
                }}>
                Ready →
              </button>
            </div>
          ...
          {/* SPEAKING state */}
          {isTalking && (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, color: "#64748B", fontSize: 13 }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#C084FC", animation: "pulse 1s ease-in-out infinite" }} />
          ...
          {(phase === "idle" || phase === "done") && (
            <div style={{ textAlign: "center", color: "#475569", fontSize: 13 }}>
              {phase === "done" ? "⏳ Generating your feedback…" : "Preparing next question…"}
            </div>
          )}
        </div>
      </div>
  ...
  if (screen === "feedback") {
    if (!feedback || feedback.error) return (
      <div style={{ ...app, display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ textAlign: "center", padding: 40 }}>
          ...
          <p style={{ color: "#64748B", marginBottom: 20 }}>Your exam was complete — please try again.</p>
          <button onClick={() => setScreen("home")} style={{ background: "#3B82F6", color: "#fff", border: "none", padding: "12px 28px", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: "pointer" }}>
            Return Home
          </button>
        </div>
      </div>
    ...
    const TABS = ["overview", "language", "expressions"];
        ...
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{
              ...
              background: "#141B24", border: "1px solid #1E2D40", borderRadius: 18,
              padding: "18px 28px", marginTop: 18, marginBottom: 16,
            }}>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 60, fontWeight: 900, color: bc(overallBand), lineHeight: 1, letterSpacing: -2 }}>
                  {overallBand}
                </div>
                <div style={{ color: "#475569", fontSize: 11, textTransform: "uppercase", letterSpacing: 1, marginTop: 4 }}>
                  Overall Band
                </div>
              </div>
              <div style={{ borderLeft: "1px solid #1E2D40", paddingLeft: 20 }}>
                {[9, 8, 7, 6, 5, 4].map((n) => (
                  <div key={n} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
                    ...
                    <span style={{ fontSize: 11, color: n <= Math.round(overallBand) ? "#94A3B8" : "#2D3748" }}>
                      {n === 9 ? "Expert User" : n === 8 ? "Very Good User" : n === 7 ? "Good User" : n === 6 ? "Competent User" : n === 5 ? "Modest User" : "Limited User"}
                    </span>
                  </div>
                ))}
              </div>
            ...
            {examinerNote && (
              <p style={{ color: "#94A3B8", fontSize: 14, lineHeight: 1.75, maxWidth: 520, margin: "0 auto" }}>
                {examinerNote}
              </p>
            )}
          </div>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 20, background: "#141B24", borderRadius: 12, padding: 4 }}>
            {[
              ...
              { id: "expressions", label: "Natural Speech" },
            ].map((t) => (
              <button key={t.id} onClick={() => setActiveTab(t.id)}
                style={{
                  flex: 1, padding: "9px 8px", border: "none", borderRadius: 10, cursor: "pointer",
                  background: activeTab === t.id ? "#0C1117" : "transparent",
                  color: activeTab === t.id ? "#E2E8F0" : "#475569",
                  fontSize: 13, fontWeight: 600, outline: "none", transition: "all .15s",
                }}>
                {t.label}
              </button>
            ))}
            ...
            <div style={{ animation: "slideUp .2s ease" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
                {criteriaList.map((c, i) => (
                  <div key={i} style={{
                    background: "#141B24", border: "1px solid #1E2D40", borderRadius: 12,
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ height: 4, background: "#1E2D40", borderRadius: 2 }}>
                        <div style={{ width: `${(c.band / 9) * 100}%`, height: "100%", background: bc(c.band), borderRadius: 2 }} />
                      </div>
                    </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
                <div style={{ background: "#091510", border: "1px solid #163020", borderRadius: 14, padding: 16 }}>
                  ...
                  {strengths.map((s, i) => (
                    <div key={i} style={{ fontSize: 12, color: "#94A3B8", marginBottom: 8, paddingLeft: 10, borderLeft: "2px solid #4ADE80", lineHeight: 1.6 }}>{s}</div>
                  ))}
                </div>
                <div style={{ background: "#130A0A", border: "1px solid #301515", borderRadius: 14, padding: 16 }}>
                  ...
                  {improvements.map((s, i) => (
                    <div key={i} style={{ fontSize: 12, color: "#94A3B8", marginBottom: 8, paddingLeft: 10, borderLeft: "2px solid #F87171", lineHeight: 1.6 }}>{s}</div>
                  ))}
                </div>
              </div>
              ...
              {/* Closing */}
              {closing && (
                <div style={{ background: "#141B24", border: "1px solid #3B82F633", borderRadius: 14, padding: 18, textAlign: "center" }}>
                  <div style={{ fontSize: 22, marginBottom: 8 }}>⭐</div>
                  <p style={{ color: "#94A3B8", fontSize: 14, lineHeight: 1.8, margin: 0 }}>{closing}</p>
                </div>
              )}
            </div>
          )}

          {/* ── Tab: Language (criteria detail) ── */}
          {activeTab === "language" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14, animation: "slideUp .2s ease" }}>
              {criteriaList.map((c, i) => (
                <div key={i} style={{ background: "#141B24", border: "1px solid #1E2D40", borderRadius: 14, padding: 18 }}>
                  ...
                  <div style={{ height: 5, background: "#1E2D40", borderRadius: 3, marginBottom: 14 }}>
                    <div style={{ width: `${(c.band / 9) * 100}%`, height: "100%", background: bc(c.band), borderRadius: 3 }} />
                  </div>
                  <p style={{ color: "#94A3B8", fontSize: 13, lineHeight: 1.7, marginBottom: 14 }}>{c.comment}</p>

                  {c.tips?.length > 0 && (
                    <div style={{ marginBottom: 14 }}>
                      <div style={{ fontSize: 11, color: "#475569", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Tips</div>
                      {c.tips.map((t, j) => (
                  ...
                  {c.upgrades?.length > 0 && (
                    <div style={{ marginBottom: 10 }}>
                      <div style={{ fontSize: 11, color: "#475569", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Vocabulary Upgrades</div>
                      {c.upgrades.map((u, j) => (
                        <div key={j} style={{ background: "#0A1220", borderRadius: 10, padding: "10px 12px", marginBottom: 8 }}>
                          <div style={{ color: "#F87171", fontSize: 13, marginBottom: 4 }}>You said: <em>"{u.used}"</em></div>
                          <div style={{ color: "#4ADE80", fontSize: 13 }}>Better: <strong>"{u.better}"</strong></div>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {c.corrections?.length > 0 && (
                    <div>
                      <div style={{ fontSize: 11, color: "#475569", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Grammar Corrections</div>
                      {c.corrections.map((x, j) => (
                        <div key={j} style={{ background: "#0A1220", borderRadius: 10, padding: "10px 12px", marginBottom: 8 }}>
                          <div style={{ color: "#F87171", fontSize: 13, marginBottom: 4 }}>❌ <em>"{x.wrong}"</em></div>
                          <div style={{ color: "#4ADE80", fontSize: 13, marginBottom: x.rule ? 6 : 0 }}>✅ <strong>"{x.correct}"</strong></div>
                          {x.rule && <div style={{ color: "#475569", fontSize: 11 }}>💡 {x.rule}</div>}
                        </div>
                      ))}
                    </div>
                  )}
            ...
            <div style={{ animation: "slideUp .2s ease" }}>
              {naturalSpeech.length === 0 ? (
                <div style={{ textAlign: "center", padding: 48, color: "#475569" }}>
                  No natural speech suggestions for this session.
                </div>
              ) : (
                <div>
                  <p style={{ color: "#64748B", fontSize: 13, lineHeight: 1.65, marginBottom: 20 }}>
                    These are real examples from your answers, rewritten to sound more natural and fluent — the kind of language that gets higher band scores.
                  </p>
                  {naturalSpeech.map((e, i) => (
                    <div key={i} style={{
                      background: "#141B24", border: "1px solid #1E2D40", borderRadius: 14,
                      padding: 18, marginBottom: 14, animation: "slideUp .2s ease",
                    }}>
                      <div style={{ color: "#64748B", fontSize: 12, marginBottom: 12, fontStyle: "italic" }}>
                        📌 {e.context}
                      </div>
                      ...
                      {e.why && (
                        <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                          ...
                          <span style={{ color: "#64748B", fontSize: 12, lineHeight: 1.6 }}>{e.why}</span>
                        </div>
                      )}
                    </div>
                  ))}
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
            <button
              onClick={() => { stopAll(); setScreen("home"); setFeedback(null); setMsgs([]); }}
              style={{
                flex: 1, padding: 16, fontSize: 15, fontWeight: 800, border: "none",
            ...
            <button
              onClick={() => { setFeedback(null); setMsgs([]); generateExam(topicId, freestyle); }}
              style={{
                padding: "16px 18px", fontSize: 13, fontWeight: 600,
                border: "1px solid #1E2D40", borderRadius: 12, cursor: "pointer",
                background: "#141B24", color: "#94A3B8", outline: "none",
              }}>
              Retry Topic
            </button>
          </div>
